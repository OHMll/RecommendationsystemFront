  ...
  const fetchData = async () => {
    try {
      ...
      const res = await fetch(
        "http://localhost:8000/api/recommend-spaceguides",
        {
          credentials: "include",
        }
      );
      const data = await res.json();
      console.log("Data from recommend-spaceguides:", data);

      // แก้ตรงนี้ให้มี || เชื่อมเงื่อนไข
      if (
        !data ||
        data.message === "Authentication required" ||
        (Array.isArray(data) && data.length === 0)
      ) {
        const fallbackRes = await fetch("http://localhost:8000/api/sginfo");
        const fallbackData = await fallbackRes.json();
        console.log("Fallback data from sginfo:", fallbackData);

        setSpaceData(fallbackData);
      } else {
        setSpaceData(data);
      }
    } catch (error) {
